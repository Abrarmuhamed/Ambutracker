...
$('.price-range').change(function () {
    var value = $(this).val();
    $(".slider-val").text(value);
    $("#price").val(value);
});

$(".upload-img").click(function () {
    var item = $(this).attr("this-for");
    $("#" + item).click();
})

$(".img-upload").change(function () {
    var itemBox = $(this).attr("this-box");
    var item = $(this).attr("this-for");
...
$("#state").change(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            const arr = response.split("eleksa");
            $(".price-range").attr("min", arr[0]);
            $("#min-range").val(arr[0]);
            $(".price-range").attr("max", arr[1]);
            $("#max-range").val(arr[1]);
        }
    })
})
...
$("#change_number").click(function () {
    $.ajax({
        url: "functions/functions.php",
        type: "post",
        data: { change_number: true },
        success: function (response) {
            if (response == 1) {
                window.location.href = "http://localhost/electric/";
            }
        }
    })
})

$("body").on("click", "#resend-otp", function () {
    if (confirm("Are you sure to resend otp?")) {
        $.ajax({
            ...
            success: function (html) {
                $("#resend-div").html(html).show();
            },
        });
    }
});

// $(".duration-input").click(function () {
//     var start = $("#time-start").val();
//     var end = $("#time-to").val();

//     var balance = (new Date("1970-1-1 " + start) - new Date("1970-1-1 " + end)) / 1000 / 60 / 60;;
//     alert(balance)
// })

$(".type-btn").click(function () {
    $(".type-btn").removeClass("btn-theme");
